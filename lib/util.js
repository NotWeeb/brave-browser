...
const util = {
  ...
  buildGClientConfig: () => {
    ...
    let cache_dir = process.env.GIT_CACHE_PATH ? ('\ncache_dir = "' + process.env.GIT_CACHE_PATH + '_1.7"\n') : '\n'
...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  gclientSync: (reset = false, options = {}) => {
    if (fs.existsSync(config.srcDir) &&
      ...
      util.run('git', ['-C', config.srcDir, 'fetch', 'origin'], options)
    }
    if (fs.existsSync(config.braveCoreDir) &&
        util.run('git', ['-C', config.braveCoreDir, 'config', '--get', 'remote.origin.url']).stdout.toString().trim() != config.braveCoreRepo) {
      console.log('setting src/brave remote `origin` to ' + config.braveCoreRepo)
      util.run('git', ['-C', config.braveCoreDir, 'remote', 'remove', 'origin'], options)
      util.run('git', ['-C', config.braveCoreDir, 'remote', 'add', 'origin', config.braveCoreRepo], options)
      util.run('git', ['-C', config.braveCoreDir, 'fetch', 'origin'], options)
    }
